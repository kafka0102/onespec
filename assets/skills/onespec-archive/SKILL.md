...
若用户确认无问题，不要再要求输入 `continue`、`yes`、`批准收尾` 等完整词；直接展示收尾编号选项，用户回复数字即可。若意图不在选项中，允许用户直接补充说明其他操作。

不要让用户自己猜“下一步该输入什么”。进入 `onespec-archive` 时，必须给出可直接回复的编号选项；如果支持多动作组合，允许用户回复逗号分隔的数字，例如 `3,5`。
...
- PR/MR：根据仓库托管平台提示用户下一步。如果 remote 明确是 GitHub，使用 `PR`；如果是 GitLab，使用 `MR`；无法判断时写成 `PR/MR`。
- 保留：不合并不删除，状态仍可标记 `done`。

不要默认自动合并 worktree 到 `main`，也不要默认删除 worktree。合并、PR/MR、删除都是有后果的操作，必须来自用户选择。
...
收尾选项不要再只做单选。至少把以下两个动作设计成可多选组合；组合输入使用编号列表，例如 `3,5`：
...
5. 若代码已真正位于目标分支，可执行归档
其他：如果意图不在以上选项里，允许用户直接补充说明

菜单解释规则：

- 用户回复 `1`：继续停留在 review 状态，不做收尾。
- 用户回复 `2`：进入推送分支与创建 `PR/MR` 的流程。
- 用户回复 `3`：进入本地合并与清理临时 branch/worktree 的流程。
- 用户回复 `4`：保留当前状态，标记 `done` 或维持可继续状态，等待之后再处理。
- 用户回复 `5`：仅当代码已合并且归档前置条件满足时，执行 archive；否则先说明阻塞条件。
- 用户回复多项编号，如 `3,5`：按组合规则校验。合法时依次执行；不合法时明确指出冲突原因。
