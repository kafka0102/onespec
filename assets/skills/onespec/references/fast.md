...
供 `onespec` 和独立 `onespec-fast` 入口在 `fast` 路径按需读取。目标是省掉常规 proposal 批准 gate 和实现完成后的人工归档选择，但只在复杂度检查确认低复杂度时这样做。
...
- 勾选 `tasks.md` 中已完成的任务。
- 如实现暴露新的设计冲突，停止自动路径，修正 OpenSpec artifacts，并转入常规 design phase / execute phase gate。
- 运行项目测试。
...
- 使用 design phase 的 Proposal 批准 Gate 与路径选择菜单，让用户显式批准 proposal / design / spec 后再进入实现。
